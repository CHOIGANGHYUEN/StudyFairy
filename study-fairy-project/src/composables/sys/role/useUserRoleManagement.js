...
export function useUserRoleManagement() {
  ...
  const mappings = ref([]);
  const roles = ref([]);
  const form = ref({ userId: "", roleId: "", useYn: 1 });
  ...
  const fetchMappings = async (page = 1) => {
    try {
      ...
      if (res.data?.data) {
        ...
        totalPages.value = Math.ceil(res.data.length / pageSize) || 1;
        mappings.value = res.data.slice((page - 1) * pageSize, page * pageSize);
      } else {
        mappings.value = [];
        totalPages.value = 1;
        totalCount.value = 0;
  ...
  const handlePageChange = (page) => fetchMappings(page);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditMode.value) await updateUserRole(editTargetId.value, payload);
      else await createUserRole(payload);
