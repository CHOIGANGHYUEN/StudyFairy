...
export function useSyst004v001() {
  ...
  const handleDelete = async () => {
    const targetIds =
      selectedRows.value.length > 0
        ? selectedRows.value
        : selectedRow.value
          ? [selectedRow.value.tablen]
          : [];
    if (targetIds.length === 0 || !confirm("정말 삭제하시겠습니까?")) return;
    try {
      await Promise.all(targetIds.map((id) => deleteTable(id)));
  ...
  const handleToggleAll = (e) => {
    selectedRows.value = e.target.checked
      ? paginatedItems.value.map((item) => item.tablen)
      : [];
  };
  onMounted(fetchData);
  return {
